...
public class LogRecordKey extends org.apache.hadoop.record.Record {
	static Log LOG = LogFactory.getLog(LogRecordKey.class);
	...
	public LogRecordKey(Event e) {
		this.dataType = new String(e.get(DATA_TYPE));
		
		String t = new String(e.get(TIME));
		try {
			this.time = Long.toString(formatter.parse(t.trim()).getTime());
		} catch (ParseException e1) {
			LOG.error("Failed parse date : [" + t + "]", e1);
		}
		this.logId = new String(e.get(XmlFilter.LOG_ID));
	}
